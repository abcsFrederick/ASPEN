...
def extract_fragments(input_bam, output_tn5, output_reads, threads):
    ...
    with open(output_tn5, "w") as tn5bedoutfile, open(output_reads, "w") as readsoutfile:
        ...
        for read in bamfile.fetch(until_eof=True):  # Iterate without index lookup
            ...
            if prev_read and prev_read.query_name == read.query_name:
                ...
                read1, read2 = (prev_read, read) if prev_read.is_read1 else (read, prev_read)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract Tn5 fragment sites and reads from a BAM file.")
    parser.add_argument("-i", "--bam", required=True, help="Input BAM file (query-name sorted)")
    parser.add_argument("-t", "--tn5bed", required=True, help="Output Tn5 insertion sites BED file")
    parser.add_argument("-b", "--readsbed", required=True, help="Output reads BED file")
    parser.add_argument("-n", "--ncpus", required=False, default=2, help="Number of CPUs to use")
