...
try:
    x=pandas.read_csv("data.tss_nicking_sites.txt",header=None,sep="\t",names=["replicateName","N_TSS_gt20knicks","TSSscore"])
...
if len(dfs_to_join)>0:
    allstats=reduce(lambda a,b:pandas.merge(a,b,how="outer",on="replicateName"),dfs_to_join)
